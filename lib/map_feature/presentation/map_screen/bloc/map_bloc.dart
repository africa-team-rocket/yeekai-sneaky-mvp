...
class MapBloc extends Bloc<MapEvent, MapState> {
  ...
  void _addSearchHitToCache(
      AddSearchHitToCache event, Emitter<MapState> emit) async {
    await for (final resource
        in _addSearchHitToCacheUseCase.execute(event.searchHitEntity)) {
      switch (resource.type) {
        ...
        case ResourceType.error:
          ...
          debugPrint(
              "Oops, erreur lors de la récupération des hits :/ ${resource.message} ");
  ...
  void _applyAlgoliaState(
      ApplyAlgoliaState event, Emitter<MapState> emit) async {
    _algoliaSearchUseCase.applySearchState(event.stateUpdater, "wakagoro");
  ...
  void _getBusesNearby(GetBusesNearby event, Emitter<MapState> emit) async {
    await for (final resource
        in _getBusesNearbyUseCase.execute(event.userPos, this)) {
      switch (resource.type) {
        case ResourceType.success:
          ...
          if (busesNearby != null) {
            ...
            add(UpdateMarkersSet(
                newMarker: null,
                newMarkerSubset: busesNearby.map((e) => e.busMarker).toSet()));
          }
  ...
  void _getLocation(
      GetUserLocationUpdates event, Emitter<MapState> emit) async {
        ...
        case ResourceType.success:
          ...
          if (posInfo != null) {
            var newUserPos = UserPosition(
                // + 0.0015
                userPos: LatLng(
                    posInfo.position.latitude, posInfo.position.longitude),
                ...
                userMarker: Marker(
                  ...
                  icon: await BitmapDescriptor.fromAssetImage(
                      ...
                      Platform.isIOS
                          ? 'assets/icons/user_position_small.png'
                          : 'assets/icons/user_map_position.png'),
            ...
            add(UpdateMarkersSet(
                newMarker: newUserPos.userMarker, newMarkerSubset: null));
          }
          break;
        case ResourceType.error:
          break;
    ...
    await for (final resource in _getSearchHistoryFromCacheUseCase.execute()) {
      switch (resource.type) {
        case ResourceType.success:
          ...
          emit(state.copyWith(
              searchHistory: resource.data,
              searchLoading:
                  state.searchLoading.copyWith(isHistoryLoading: false)));
          // emit(state.copyWith(sea))
        ...
        case ResourceType.error:
          ...
          debugPrint(
              "Oops, erreur lors de la récupération de la history :/ ${resource.message} ");
        ...
        case ResourceType.loading:
          ...
          emit(state.copyWith(
              searchLoading:
                  state.searchLoading.copyWith(isHistoryLoading: true)));
    ...
    await for (final resource in _algoliaSearchUseCase.execute()) {
      switch (resource.type) {
        case ResourceType.success:
          ...
          debugPrint(resource.data.toString());
          emit(state.copyWith(
              searchHitsPage: resource.data,
              searchLoading:
                  state.searchLoading.copyWith(isSuggestionsLoading: false)));
        ...
        case ResourceType.loading:
          ...
          emit(state.copyWith(
              searchLoading:
                  state.searchLoading.copyWith(isSuggestionsLoading: true)));
  ...
  void _setGoogleMapController(
      SetGoogleMapController event, Emitter<MapState> emit) async {
  ...
  void _setSelectedMapEntity(
      SetSelectedMapEntity event, Emitter<MapState> emit) async {
    debugPrint(
        "New map entity : ${event.newMapEntity}, is it null ? ${event.newMapEntity == null}");
      ...
      if (event.newMapEntity is Place) {
        ...
        newMarker = Marker(
            ...
            icon: await BitmapDescriptor.fromAssetImage(
              const ImageConfiguration(),
              Platform.isIOS
                  ? "assets/icons/bus_stop_small.png"
        ...
        newMarker = Marker(
            ...
            icon: await BusMarkerIcon(
              ...
              state: (event.newMapEntity! as Bus).state,
                ...
                logicalSize: const Size(72, 72), imageSize: const Size(72, 72)),
            onTap: () {
              add(SetSelectedMapEntity(event.newMapEntity));
            });
        ...
        emit(state.copyWith(selectedEntity: event.newMapEntity));
        add(UpdateMarkersSet(newMarker: newMarker, newMarkerSubset: null));
        add(UpdatePolylinesSet(
            newPolyline: newPolyline, newPolylineSubset: null));
  ...
  void _updateMarkersSet(UpdateMarkersSet event, Emitter<MapState> emit) async {
    final List<Marker> newSet =
        List.from(state.markersSet); // Create a copy of the current list

    // Check if an old version of the marker is present
    if (event.newMarker != null) {
      final oldMarkerIndex = newSet
          .indexWhere((marker) => marker.markerId == event.newMarker!.markerId);
      if (oldMarkerIndex != -1) {
    ...
    if (event.newMarkerSubset != null) {
      for (var newMarker in event.newMarkerSubset!) {
        final oldMarkerIndex = newSet
            .indexWhere((marker) => marker.markerId == newMarker.markerId);
        if (oldMarkerIndex != -1) {
          newSet.removeAt(oldMarkerIndex);
        }
  ...
  void _updatePolylinesSet(
      UpdatePolylinesSet event, Emitter<MapState> emit) async {
    final List<Polyline> newSet =
        List.from(state.polylinesSet); // Create a copy of the current list

    // Check if an old version of the marker is present
    if (event.newPolyline != null) {
      final oldPolylineIndex = newSet.indexWhere(
          (polyline) => polyline.polylineId == event.newPolyline!.polylineId);
    ...
    if (event.newPolylineSubset != null) {
      for (var newPolyline in event.newPolylineSubset!) {
        final oldPolylineIndex = newSet.indexWhere(
            (polyline) => polyline.polylineId == newPolyline.polylineId);
        if (oldPolylineIndex != -1) {
          newSet.removeAt(oldPolylineIndex);
        }
  ...
  void _updateSearchFilterMode(
      UpdateSearchFilterMode event, Emitter<MapState> emit) async {
