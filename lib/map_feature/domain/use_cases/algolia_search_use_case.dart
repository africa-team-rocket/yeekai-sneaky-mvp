...
class MultimodalSearchUseCase {
  ...
  StreamController<List<SearchHitPlace>> _dataStreamController =
      StreamController<List<SearchHitPlace>>();

  Stream<List<SearchHitPlace>> get _placesStream =>
...
// Ces informations aussi doivent être cachés somewhere.
  ...
  Stream<HitsPage> get _stopsAndRoutesSearchPage =>
      Stream.fromIterable(hitsPages);

// Cette fonction vous permet de mettre à jour l'état du `_productsSearcher`.
  Future<void> applySearchState(
      SearchState Function(SearchState) stateUpdater, String query) async {
  ...
  Stream<Resource<HitsPage>> execute() {
    try {
      ...
      final combinedStream =
          Rx.combineLatest2<HitsPage, List<SearchHitPlace>, HitsPage>(
        ...
        (stopsAndRoutes, places) {
          ...
          return HitsPage(
              mergedItems, stopsAndRoutes.pageKey, stopsAndRoutes.nextPageKey);
        },
      );
